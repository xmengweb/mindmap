...
export const isWhite = (color) => {
  ...
  return (
    ['#fff', '#ffffff', '#FFF', '#FFFFFF', 'rgb(255,255,255)'].includes(color) ||
...
export const handleSelfCloseTags = (str) => {
  ;['img', 'br', 'hr', 'input', 'link', 'meta', 'area'].forEach((tagName) => {
...
// 处理获取svg内容时添加额外内容
export const handleGetSvgDataExtraContent = ({ addContentToHeader, addContentToFooter }) => {
  // 追加内容
  const cssTextList = []
  let header = null
  let headerHeight = 0
  ...
  const handle = (fn, callback) => {
    if (typeof fn === 'function') {
      const res = fn()
      if (!res) return
      const { el, cssText, height, width } = res
      if (el instanceof HTMLElement) {
        addXmlns(el)
        const foreignObject = createForeignObjectNode({ el, height, width })
...
// 获取多个节点总的包围框
export const getNodeListBoundingRect = (nodeList, x = 0, y = 0, paddingX = 0, paddingY = 0) => {
  let minX = Infinity
  let maxX = -Infinity
  let minY = Infinity
  let maxY = -Infinity
  nodeList.forEach((node) => {
    const { left, top, width, height } = getNodeTreeBoundingRect(node, x, y, paddingX, paddingY, false, true)
    if (left < minX) {
      minX = left
    }
    if (left + width > maxX) {
