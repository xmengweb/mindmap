...
export class Event extends EventEmitter {
  ...
  //  鼠标移动事件
  onMousemove(e) {
    let { useLeftKeySelectionRightKeyDrag = false } = this.mindMap.opt
  ...
  onMousewheel(e) {
    ...
    e.preventDefault()
    const dirs = []
    if (e.deltaY < 0) dirs.push('Up')
    if (e.deltaY > 0) dirs.push('Down')
    if (e.deltaX < 0) dirs.push('Left')
    if (e.deltaX > 0) dirs.push('Right')
    // 判断是否是触控板
