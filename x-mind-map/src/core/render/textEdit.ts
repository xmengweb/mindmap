...
export default class TextEdit {
  ...
  bindEvent() {
    ...
    // 节点双击事件
    this.mindMap.event.on('node_dblclick', (node, e, isInserting) => {
      this.show({ node, e, isInserting })
    })
    // 点击事件
    this.mindMap.event.on('draw_click', () => {
      // 隐藏文本编辑框
      this.hideEditTextBox()
    })
    this.mindMap.event.on('body_mousedown', () => {
      this.hasBodyMousedown = true
    })
    this.mindMap.event.on('body_click', () => {
    ...
    this.mindMap.event.on('svg_mousedown', () => {
      // 隐藏文本编辑框
      this.hideEditTextBox()
    })
    // 展开收缩按钮点击事件
    this.mindMap.event.on('expand_btn_click', () => {
      this.hideEditTextBox()
    })
    // 节点激活前事件
    this.mindMap.event.on('before_node_active', () => {
      this.hideEditTextBox()
    })
    // 鼠标滚动事件
    this.mindMap.event.on('mousewheel', () => {
      if (this.mindMap.opt.mousewheelAction === 'move') {
        this.hideEditTextBox()
      }
    })

    this.mindMap.event.on('scale', this.onScale)
    // // 监听按键事件，判断是否自动进入文本编辑模式
    if (this.mindMap.opt.enableAutoEnterTextEditWhenKeydown) {
      window.addEventListener('keydown', this.onKeydown)
    }
    this.mindMap.event.on('beforeDestroy', () => {
      this.unBindEvent()
    })
  }
  ...
  hideEditTextBox() {
    ...
    this.mindMap.emit('hide_text_edit', this.textEditNode, this.renderer.activeNodeList, this.currentNode)
    this.currentNode = null
